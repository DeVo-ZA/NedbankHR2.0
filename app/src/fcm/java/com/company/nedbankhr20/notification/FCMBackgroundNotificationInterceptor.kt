...
open class FCMBackgroundNotificationInterceptor(context: Context, badgeServiceChannel: NotificationChannel) : BasePushService.BackgroundNotificationInterceptor {
    lateinit var notificationPreferences: SharedPreferences
    private val mBadgeServiceChannel = badgeServiceChannel
    val mContext = context
    override fun onBackgroundNotificationMessageReceived(pushEvent: BasePushService.PushNotificationEvent) {
        ...
        if (pushEvent.pushRemoteMessage!!.isBadgeMessage) {
            val messageData = pushEvent.pushRemoteMessage!!.data
        ...
        val badgeNumber = pushEvent.pushRemoteMessage!!.badge
        val mesgAlert = pushEvent.pushRemoteMessage!!.alert
        if (isBadgeOnlyMessage) {
            pushEvent.pushRemoteMessage!!.alert = mesgAlert?: "$badgeNumber messages received"
            pushEvent.displayNotification(pushEvent.pushRemoteMessage)
        } else if (!isBadgeOnlyMessage && pushEvent.pushRemoteMessage!!.isBadgeMessage) {
            // update the notification as a data only notification
            val dRemoteMessage = pushEvent.pushRemoteMessage
//            dRemoteMessage!!.setIsBadgeMessage(false)
            ...
            if (dRemoteMessage?.data?.containsKey("businessId")!!) {
                ...
                pushEvent.displayNotification(dRemoteMessage)
            ...
            val badgeMessage = pushEvent.pushRemoteMessage
            ...
            notificationTrampolineActivityIntent.run {
                ...
                putExtra(BasePushService.NOTIFICATION_CONDITION,pushEvent.foregroundPushNotificationReady)
            }
            ...
            builder?.build()?.let { if (ActivityCompat.checkSelfPermission(mContext, Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
